...
const Login = () => {
  ...
  // If user is already authenticated, redirect immediately away from login
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        else if (role === 'shopkeeper') navigate('/shopkeeper', { replace: true });
        else if (role === 'ngo') navigate('/ngo-command', { replace: true });
    ...
    try {
      ...
      if (response.data.token) {
        ...
        if (userRole === 'admin') {
          ...
          navigate('/ngo-command', { replace: true });
        } else {
          navigate('/dashboard', { replace: true });
        }
      }
          ...
          <div className="glass-container rounded-[32px] p-8 lg:p-10 shadow-[0_32px_80px_rgba(0,0,0,0.6)] relative z-10">
            ...
            <div className="flex items-center justify-between mb-4">
              <button
                ...
                className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-400 hover:text-emerald-500 dark:hover:text-[#00ff87] transition-colors cursor-pointer active:scale-95"
              >
