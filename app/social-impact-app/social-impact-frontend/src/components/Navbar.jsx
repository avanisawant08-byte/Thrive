  ...
  return (
    <>
      ...
      <nav className="md:hidden bg-white/95 dark:bg-zinc-900/90 backdrop-blur-[20px] border-t border-slate-200/80 dark:border-white/5 fixed bottom-0 left-0 w-full flex justify-around items-center px-1.5 py-2.5 pb-safe z-50 rounded-t-[28px] shadow-[0_-10px_30px_rgba(15,23,42,0.08)] dark:shadow-[0_-10px_40px_rgba(0,0,0,0.5)] transition-colors">
        <Link to={user?.role === 'ngo' ? '/ngo-command' : '/dashboard'} className={`flex flex-col items-center justify-center py-1 px-1 transition-all ${path === '/dashboard' || path === '/ngo-command' ? 'text-emerald-600 dark:text-[#00ff87]' : 'text-slate-500 dark:text-zinc-500 hover:text-slate-900 dark:hover:text-[#f1ffef]'}`}>
          <span className="material-symbols-outlined text-xl">home</span>
          <span className="font-inter text-[9px] uppercase tracking-wider font-bold">Home</span>
        </Link>

        <Link to="/events" className={`flex flex-col items-center justify-center py-1 px-1 transition-all ${path === '/events' ? 'text-emerald-600 dark:text-[#00ff87]' : 'text-slate-500 dark:text-zinc-500 hover:text-slate-900 dark:hover:text-[#f1ffef]'}`}>
