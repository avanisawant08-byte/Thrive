...
const EventDetail = ({ event, onClose }) => {
  ...
  return (
    <div
      className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md overflow-hidden"
      onClick={onClose}
    >
      <div
        className="w-full sm:max-w-2xl bg-white dark:bg-surface rounded-t-[2rem] sm:rounded-[2rem] border border-slate-200/90 dark:border-white/10 shadow-2xl overflow-hidden max-h-[92vh] sm:max-h-[90vh] flex flex-col transition-colors mb-0"
      ...
      >
        ...
        {/* Banner */}
        <div className="relative h-48 sm:h-56 flex-shrink-0 overflow-hidden bg-slate-900">
          <img
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-white dark:from-surface via-transparent to-black/60" />

          {/* Top Control Bar: Back Arrow & Close */}
          <div className="absolute top-3.5 left-3.5 right-3.5 z-20 flex items-center justify-between pointer-events-none">
            <div className="flex items-center gap-2 pointer-events-auto">
              <button
                type="button"
                onClick={onClose}
                className="w-9 h-9 bg-black/60 hover:bg-black/80 active:scale-90 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-all shadow-lg cursor-pointer"
                title="Back to Events"
              >
                <span className="material-symbols-outlined text-xl">arrow_back</span>
              </button>
              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border backdrop-blur-md shadow-md ${meta.color}`}>
            ...
            <button
              type="button"
              onClick={onClose}
              className="pointer-events-auto w-9 h-9 bg-black/60 hover:bg-black/80 active:scale-90 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-all shadow-lg cursor-pointer"
              title="Close"
            >
          ...
          <div className="absolute bottom-3 right-4 z-10">
            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border backdrop-blur-md flex items-center gap-1 shadow-md ${statusStyle.chip}`}>
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          <div className="p-5 sm:p-6 space-y-5">
            ...
            <div className="flex items-start justify-between gap-3">
              <h2 className="text-xl sm:text-2xl font-black tracking-tight leading-tight flex-1 text-slate-900 dark:text-on-surface">{localEvent.title}</h2>
              {localEvent.coinsReward > 0 && (
                <div className="flex-shrink-0 flex items-center gap-1.5 bg-emerald-50 dark:bg-primary-container/10 border border-emerald-300/80 dark:border-primary-container/20 px-3 py-1 rounded-full shadow-sm">
                  ...
                  <span className="text-xs font-black text-emerald-700 dark:text-primary-container">+{localEvent.coinsReward}</span>
                </div>
              )}
            </div>
        ...
        {/* Footer CTA */}
        <div className="p-4 pb-8 sm:pb-4 border-t border-slate-200/80 dark:border-white/5 flex-shrink-0 bg-slate-50/95 dark:bg-surface/95 flex flex-col gap-2 backdrop-blur-md">
          {canCancel && (
            <button
              onClick={() => setShowCancelModal(true)}
              className="w-full py-3.5 rounded-2xl font-black text-sm text-rose-600 dark:text-red-500 bg-rose-50 dark:bg-red-500/10 hover:bg-rose-100 dark:hover:bg-red-500/20 transition-all border border-rose-200 dark:border-red-500/20 active:scale-95 cursor-pointer"
            >
              Cancel Event
            </button>
          )}
          ...
          {!isCreator && (
            !isCompleted && !isCancelled && !isRejected && spotsLeft !== 0 ? (
              <button
                ...
                className={`w-full py-3.5 sm:py-4 rounded-2xl font-black text-sm transition-all active:scale-95 cursor-pointer ${
                  joined
                    ? 'bg-slate-200 dark:bg-surface-container-highest text-slate-700 dark:text-on-surface-variant cursor-default'
              ...
              <div className="w-full py-3.5 sm:py-4 rounded-2xl bg-slate-200/90 dark:bg-surface-container-low text-slate-700 dark:text-on-surface-variant font-black text-xs sm:text-sm text-center border border-slate-300 dark:border-white/5 shadow-inner">
      ...
      {showCancelModal && (
        <div 
          className="fixed inset-0 bg-black/70 backdrop-blur-md z-[160] flex items-center justify-center p-4"
          onClick={(e) => e.stopPropagation()}
        >
          <div 
